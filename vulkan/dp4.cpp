...
lm_status lm_vulkan_dot_i8_dp4(const char *spv_path, uint32_t device_index,
    ...
    if (!spv_path || !a || !b || !out_result || packed_words == 0u) return LM_ERR_ARGUMENT;
    ...
    if (vkEnumeratePhysicalDevices(instance, &device_count, devices.data()) != VK_SUCCESS)
        return fail(LM_ERR_UNSUPPORTED);
    ...
    if (vkCreateComputePipelines(device, VK_NULL_HANDLE, 1u, &compute_info, nullptr, &pipeline) != VK_SUCCESS)
        return fail(LM_ERR_UNSUPPORTED);

    const VkDeviceSize sizes[3] = {packed_words * sizeof(uint32_t), packed_words * sizeof(uint32_t), sizeof(int32_t)};
    for (uint32_t i = 0u; i < 3u; ++i) {
        VkBufferCreateInfo buffer_info{};
        buffer_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
        buffer_info.size = sizes[i];
        buffer_info.usage = VK_BUFFER_USAGE_STORAGE_BUFFER_BIT;
        if (vkCreateBuffer(device, &buffer_info, nullptr, &buffers[i]) != VK_SUCCESS)
            return fail(LM_ERR_CAPACITY);
        VkMemoryRequirements requirements{};
        vkGetBufferMemoryRequirements(device, buffers[i], &requirements);
        const uint32_t type = find_memory_type(physical, requirements.memoryTypeBits,
        ...
        if (vkAllocateMemory(device, &allocate_info, nullptr, &memories[i]) != VK_SUCCESS)
            return fail(LM_ERR_CAPACITY);
        if (vkBindBufferMemory(device, buffers[i], memories[i], 0u) != VK_SUCCESS)
            return fail(LM_ERR_STATE);
    }
    void *mapped = nullptr;
    if (vkMapMemory(device, memories[0], 0u, sizes[0], 0u, &mapped) != VK_SUCCESS) return fail(LM_ERR_STATE);
    std::memcpy(mapped, a, static_cast<size_t>(sizes[0]));
    vkUnmapMemory(device, memories[0]);
    if (vkMapMemory(device, memories[1], 0u, sizes[1], 0u, &mapped) != VK_SUCCESS) return fail(LM_ERR_STATE);
    std::memcpy(mapped, b, static_cast<size_t>(sizes[1]));
    vkUnmapMemory(device, memories[1]);
    if (vkMapMemory(device, memories[2], sizes[2] - sizeof(int32_t), sizeof(int32_t), 0u, &mapped) != VK_SUCCESS)
        return fail(LM_ERR_STATE);
    std::memset(mapped, 0, sizeof(int32_t));
    ...
    vkCmdPushConstants(command_buffer, pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0u, sizeof(packed_words), &packed_words);
    ...
    if (vkQueueSubmit(queue, 1u, &submit, VK_NULL_HANDLE) != VK_SUCCESS) return fail(LM_ERR_STATE);
    if (vkQueueWaitIdle(queue) != VK_SUCCESS) return fail(LM_ERR_STATE);
    if (vkMapMemory(device, memories[2], 0u, sizeof(int32_t), 0u, &mapped) != VK_SUCCESS)
        return fail(LM_ERR_STATE);
    std::memcpy(out_result, mapped, sizeof(int32_t));
    vkUnmapMemory(device, memories[2]);
    cleanup();
    return LM_OK;
}
