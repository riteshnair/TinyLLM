...
void destroy_buffer(lm_vulkan_packed_context *context, uint32_t index) {
...
void cleanup(lm_vulkan_packed_context *context) {
    if (!context) return;
...
lm_status ensure_buffer(lm_vulkan_packed_context *context, uint32_t index, VkDeviceSize bytes) {
    ...
    if (vkCreateBuffer(context->device, &buffer_info, nullptr, &context->buffers[index]) != VK_SUCCESS) return LM_ERR_CAPACITY;
    ...
    if (vkAllocateMemory(context->device, &allocation, nullptr, &context->memories[index]) != VK_SUCCESS) return LM_ERR_CAPACITY;
    if (vkBindBufferMemory(context->device, context->buffers[index], context->memories[index], 0u) != VK_SUCCESS) return LM_ERR_STATE;
...
lm_status lm_vulkan_packed_context_create(const char *spv_path, uint32_t device_index,
    ...
    if (!spv_path || !out_context || block_bytes == 0u || block_values == 0u) return LM_ERR_ARGUMENT;
    *out_context = nullptr;
    std::vector<uint32_t> code;
    if (!read_spv(spv_path, &code)) return LM_ERR_IO;
    lm_vulkan_packed_context *context = new (std::nothrow) lm_vulkan_packed_context{};
    if (!context) return LM_ERR_CAPACITY;
    context->queue_family = UINT32_MAX;
    context->block_bytes = block_bytes;
    context->block_values = block_values;
    ...
    uint32_t count = 0u;
    if (vkEnumeratePhysicalDevices(context->instance, &count, nullptr) != VK_SUCCESS || device_index >= count) { cleanup(context); return device_index >= count ? LM_ERR_RANGE : LM_ERR_UNSUPPORTED; }
    ...
    for (uint32_t i = 0u; i < queue_count; ++i)
        if (queues[i].queueCount != 0u && (queues[i].queueFlags & VK_QUEUE_COMPUTE_BIT) != 0u) { context->queue_family = i; break; }
    ...
    for (uint32_t i = 0u; i < 3u; ++i) { bindings[i].binding = i; bindings[i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; bindings[i].descriptorCount = 1u; bindings[i].stageFlags = VK_SHADER_STAGE_COMPUTE_BIT; }
    ...
    if (vkCreateDescriptorPool(context->device, &pool_info, nullptr, &context->descriptor_pool) != VK_SUCCESS) { cleanup(context); return LM_ERR_CAPACITY; }
...
void lm_vulkan_packed_context_destroy(lm_vulkan_packed_context *context) { cleanup(context); }
...
lm_status lm_vulkan_packed_context_matvec(lm_vulkan_packed_context *context,
                                          ...
                                          float *out) {
    if (!context || context->device == VK_NULL_HANDLE || !packed || !input || !out || rows == 0u || blocks_per_row == 0u) return LM_ERR_ARGUMENT;
    const uint64_t packed_bytes_64 = static_cast<uint64_t>(rows) * blocks_per_row * context->block_bytes;
    const uint64_t input_bytes_64 = static_cast<uint64_t>(blocks_per_row) * context->block_values * sizeof(float);
    const uint64_t output_bytes_64 = static_cast<uint64_t>(rows) * sizeof(float);
    if (packed_bytes_64 > static_cast<uint64_t>(std::numeric_limits<VkDeviceSize>::max()) ||
        input_bytes_64 > static_cast<uint64_t>(std::numeric_limits<VkDeviceSize>::max()) ||
        output_bytes_64 > static_cast<uint64_t>(std::numeric_limits<VkDeviceSize>::max())) return LM_ERR_CAPACITY;
    const VkDeviceSize sizes[3] = {static_cast<VkDeviceSize>(packed_bytes_64), static_cast<VkDeviceSize>(input_bytes_64), static_cast<VkDeviceSize>(output_bytes_64)};
    for (uint32_t i = 0u; i < 3u; ++i) { const lm_status status = ensure_buffer(context, i, sizes[i]); if (status != LM_OK) return status; }
    void *mapped = nullptr;
    if (vkMapMemory(context->device, context->memories[0], 0u, sizes[0], 0u, &mapped) != VK_SUCCESS) return LM_ERR_STATE;
    std::memcpy(mapped, packed, static_cast<size_t>(sizes[0]));
    vkUnmapMemory(context->device, context->memories[0]);
    if (vkMapMemory(context->device, context->memories[1], 0u, sizes[1], 0u, &mapped) != VK_SUCCESS) return LM_ERR_STATE;
    std::memcpy(mapped, input, static_cast<size_t>(sizes[1]));
    vkUnmapMemory(context->device, context->memories[1]);
    if (vkMapMemory(context->device, context->memories[2], 0u, sizes[2], 0u, &mapped) != VK_SUCCESS) return LM_ERR_STATE;
    std::memset(mapped, 0, static_cast<size_t>(sizes[2]));
    vkUnmapMemory(context->device, context->memories[2]);
    VkDescriptorBufferInfo infos[3] = {};
    VkWriteDescriptorSet writes[3] = {};
    for (uint32_t i = 0u; i < 3u; ++i) { infos[i].buffer = context->buffers[i]; infos[i].range = sizes[i]; writes[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET; writes[i].dstSet = context->descriptor_set; writes[i].dstBinding = i; writes[i].descriptorCount = 1u; writes[i].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER; writes[i].pBufferInfo = &infos[i]; }
    ...
    const uint32_t push[2] = {rows, blocks_per_row};
    vkCmdBindPipeline(context->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, context->pipeline);
    vkCmdBindDescriptorSets(context->command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, context->pipeline_layout, 0u, 1u, &context->descriptor_set, 0u, nullptr);
    vkCmdPushConstants(context->command_buffer, context->pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0u, sizeof(push), push);
    ...
    if (vkMapMemory(context->device, context->memories[2], 0u, sizes[2], 0u, &mapped) != VK_SUCCESS) return LM_ERR_STATE;
    std::memcpy(out, mapped, static_cast<size_t>(sizes[2]));
    vkUnmapMemory(context->device, context->memories[2]);
