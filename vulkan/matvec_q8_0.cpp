...
lm_status lm_vulkan_matvec_q8_0(const char *spv_path, uint32_t device_index,
                                ...
                                float *out) {
    if (!spv_path || !packed_q8_0 || !input || !out || rows == 0u || blocks_per_row == 0u) return LM_ERR_ARGUMENT;
    const uint64_t row_bytes_64 = static_cast<uint64_t>(blocks_per_row) * 34u;
    const uint64_t packed_bytes_64 = row_bytes_64 * rows;
    const uint64_t input_bytes_64 = static_cast<uint64_t>(blocks_per_row) * 32u * sizeof(float);
