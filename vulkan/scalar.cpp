...
lm_status lm_vulkan_dot_f32(const char *spv_path, uint32_t device_index,
    ...
    if (!spv_path || !a || !b || !out_result || count == 0u) return LM_ERR_ARGUMENT;
    ...
    for (uint32_t i = 0u; i < queue_count; ++i) {
        if (queue_props[i].queueCount != 0u && (queue_props[i].queueFlags & VK_QUEUE_COMPUTE_BIT) != 0u) { queue_family = i; break; }
    }
    ...
    const VkDeviceSize input_size = static_cast<VkDeviceSize>(count) * sizeof(float);
    const VkDeviceSize sizes[3] = {input_size, input_size, sizeof(float)};
    for (uint32_t i = 0u; i < 3u; ++i) {
    ...
    void *mapped = nullptr;
    if (vkMapMemory(device, memories[0], 0u, input_size, 0u, &mapped) != VK_SUCCESS) return fail(LM_ERR_STATE);
    std::memcpy(mapped, a, static_cast<size_t>(input_size));
    vkUnmapMemory(device, memories[0]);
    if (vkMapMemory(device, memories[1], 0u, input_size, 0u, &mapped) != VK_SUCCESS) return fail(LM_ERR_STATE);
    std::memcpy(mapped, b, static_cast<size_t>(input_size));
    ...
    vkCmdPushConstants(command_buffer, pipeline_layout, VK_SHADER_STAGE_COMPUTE_BIT, 0u, sizeof(count), &count);
